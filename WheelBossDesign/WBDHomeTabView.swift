...
protocol WBDHomeTabViewDataSource: class {
    func reloadWBDBody(focusButton: String)
}

class WBDHomeTabView: UIView {
    ...
    var lineY: CGFloat = 10
    var focusButton: String = "锻造直通" {didSet {setNeedsDisplay()}}
    let maxButtonWidth: CGFloat = 250
    
    weak var dataSource: WBDHomeTabViewDataSource?
    ...
    func clickAction(button: UIButton) {
        ...
        dataSource?.reloadWBDBody(focusButton)
    }
