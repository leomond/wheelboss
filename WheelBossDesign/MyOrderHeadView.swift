...
protocol MyOrderHeadViewDataSource: class {
    func clickHeadState(state: String)
}

class MyOrderHeadView: UIView {
    
    var head: [String] = ["全部订单", "待确认", "待收货", "待安装"]
    ...
    var focusButton: String = "全部订单" {didSet {setNeedsDisplay()}}
    
    weak var dataSource:MyOrderHeadViewDataSource?
    ...
    func clicked(button: UIButton) {
        ...
        dataSource?.clickHeadState(focusButton)
    }
